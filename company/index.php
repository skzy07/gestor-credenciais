...
$stmt = $db->prepare(
    "SELECT DISTINCT c.*, u.username AS owner_username, u.avatar_color AS owner_avatar, u.avatar_url AS owner_avatar_url,
...
<div class="page-content">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:20px;">
    <?php foreach ($companies as $company): ?>
      <div class="company-card-wrapper" style="transition:transform 0.2s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'">
        <a href="<?= APP_URL ?>/company/view.php?id=<?= $company['id'] ?>" class="card card-glow-blue" style="text-decoration:none; color:inherit; display:block; height:100%;">
          ...
          <div style="border-top:1px solid var(--glass-b);padding-top:14px;display:flex;align-items:center;justify-content:space-between;">
             <div style="display:flex;align-items:center;gap:8px">
                <div style="background:<?= sanitize($company['owner_avatar']) ?>;width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:bold;overflow:hidden;"><?php if ($company['owner_avatar_url']): ?><img src="<?= sanitize($company['owner_avatar_url']) ?>" class="avatar-img" alt="Avatar"><?php else: ?><?= sanitize(initials($company['owner_username'])) ?><?php endif; ?></div>
        ...
        <?php if ($company['is_mine']): ?>
          <a href="<?= APP_URL ?>/company/manage.php" class="btn-manage-gear" title="Gerir Empresa">⚙️</a>
        <?php endif; ?>
      </div>
    <?php endforeach; ?>
  </div>
